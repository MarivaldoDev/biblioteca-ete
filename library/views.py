...
from .forms import EmpreForm, RegisterForm, RegisterUpdateForm, UserForm
...
def criar_usuario(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            if UserStandard.objects.filter(
...
def search(request):
    ...
    context = {context_key: results, "page_obj": page_obj}
...
def emprestimos(request):
    if request.method == "POST":
        ...
        usuarios = UserStandard.objects.all()
        return render(
            request, "emprestimos.html", context={"usuarios": usuarios, "form": form}
        )

    usuarios = UserStandard.objects.all()

    return render(
        request, "emprestimos.html", context={"usuarios": usuarios, "form": EmpreForm()}
    )
